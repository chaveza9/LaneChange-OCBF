function [tf, x_e_f, v_e_f, x_f, v_f, B, i_m] = define_terminal_conditions_f_b ...
    (x_C_0, v_C_0, x_0, v_0, x_U_0, v_U_0)
       ...
       % Constraints
        ...
        T_max = 10;     % maximum allowable time
        t_avg = 3;      % Average time
        % Tunning Constants
        gamma_t = 1/T_max;
        gamma = 0.6;
        % Safety Parameters
        phi = 0.4;   % reaction time 
        delta = 15;  % Minimum safety distance
        v_d = 33;    % desired speed
        ...
        B_var = opti.variable(numCandidates-1, 1); % binary variables
        discrete = [zeros((numCandidates+1)*2+1,1);ones(numCandidates-1,1)];
        ...
        for i=2:numCandidates
            % Merge ahead of i
            opti.subject_to(x_ego_var-x_var(i,1)+(1-B_var(i-1))*M>=phi*v_var(i,1)+delta);
            % Merge behind of i
            opti.subject_to(x_var(i,1)-x_ego_var+B_var(i-1)*M>=phi*v_ego_var+delta);
        end
        ...
        % Vehicle F
        opti.subject_to(x_var(1)-x_ego_var>=phi*v_ego_var+delta);

        % Vehicle B
        opti.subject_to(x_ego_var-x_var(end)>=phi*v_var(end)+delta);
